...
def play_self_until_end_zero(
    ...
    list_next_states = [*list_states[1:], np.zeros_like(list_states[0])]
    winner = new_game.score()
    list_winner = np.ones(len(list_probs)) * winner
    return zip(list_states, list_next_states, list_probs, list_winner)  
...
def collect_self_play_data_zero(
    ...
    for _ in tqdm(range(n_games), 
                  ...
                  disable=True):
        game_data = play_self_until_end_zero(game, player=player, epsilon=epsilon)
        for state, next_state, mcts_prob, winner in game_data:
            play_data += [(state, next_state, mcts_prob, winner)]
    return play_data
...
def extend_play_data(
    ...
    extend_data = []
    for state, next_state, mcts_prob, winner in play_data:
        _, m, n = state.shape
        for i in [1, 2, 3, 4]:
            ...
            extend_data.append((equi_state, equi_next_state,
                                np.flipud(equi_mcts_prob).flatten(),
                                winner))
            # flip horizontally
            equi_state = np.array([np.fliplr(s) for s in equi_state])
            equi_next_state = np.array([np.fliplr(s) for s in next_state])
